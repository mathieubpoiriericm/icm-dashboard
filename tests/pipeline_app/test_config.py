...
class TestProviderFields:
    ...
    def test_llm_providers_constant(self):
        ...
        assert LLM_PROVIDERS == ["anthropic"]

    def test_pipeline_app_config_roundtrips_provider_field(self, tmp_config_dir):
        """Persistence round-trip for the provider field. Uses the shared
        `tmp_config_dir` fixture which redirects CONFIG_PATH to a temp dir."""
        from pipeline_app.config import load_config, save_config

        cfg = PipelineAppConfig(llm_provider="anthropic")
        save_config(cfg)
        loaded = load_config()
