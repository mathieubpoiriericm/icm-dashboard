...
#' registry IDs.
#'
#' @param table2 Data frame containing clinical trial data with Registry ID
#'   column.
#'
#' @return Character vector of unique NCT IDs (e.g., "NCT01234567").
...
#'
#' @param nct_id Character. A single NCT ID (e.g., "NCT01234567").
#' @param timeout_seconds Integer. HTTP request timeout in seconds.
#' @param max_retries Integer. Maximum number of retry attempts on failure.
#'
#' @return Data frame with columns: nct_id, facility_name, city, state, country,
...
#' @param chunk_delay_ms Integer. Delay between chunks in milliseconds.
#'
#' @return Data frame combining all location data, or empty data frame if none
#'   found.
...
#'
#' @param locations_df Data frame with city and country columns.
#'
#' @return Data frame with added lat and lon columns.
...
#' @param cache_path Path to cache file.
#'
#' @return Cached data or NULL if verification fails.
...
#'
#' @param table2 Data frame containing clinical trial data.
#' @param force_refresh Logical. If TRUE, ignores cache and fetches fresh data.
#'
#' @return Data frame with columns: nct_id, facility_name, city, state,
#'   country, lat, lon.
...
#'
#' @param status Character. The trial recruitment status.
#'
#' @return Character. CSS class name for the status badge.
...
#'
#' @param status Character. The trial recruitment status from API.
#'
#' @return Character. Formatted status text for display.
...
#'
#' @param map_data Data frame with lat and lon columns.
#' @param radius Numeric. Radius of circle in degrees (~330m at equator).
#'
#' @return Data frame with jittered coordinates for duplicates.
...
#'
#' @param locations_df Data frame with geocoded trial locations.
#' @param table2 Data frame with trial metadata for popup content.
#'
#' @return Data frame ready for leaflet rendering with popup_content column.
...
#'
#' @param map_data Data frame with map marker data.
#'
#' @return Character vector of HTML popup content.
