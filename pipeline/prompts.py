...
# ---------------------------------------------------------------------------
# V5 prompts (multi-phenotype convergence + MTAG locus scanning)
# ---------------------------------------------------------------------------

_SYSTEM_PROMPT_V5: Final[str] = _SYSTEM_PROMPT_V4  # Same system prompt

_EXTRACTION_INSTRUCTIONS_V5: Final[str] = """\
<instructions>
<task>
Extract all genes with putative causal links to cerebral small vessel disease (cSVD) from the research paper provided.
</task>
...
# ---------------------------------------------------------------------------
# Gemma 4 prompts (markdown-structured port of v4 / v5 for vLLM-served Gemma)
# ---------------------------------------------------------------------------
# Gemma 4 was trained with native `system` / `user` / `model` roles and a
# markdown-heavy data mix. The Anthropic-flavored XML scaffolding in v4 / v5
# (<instructions>, <task>, <example type="…">) is suboptimal here: it leaves
# unfamiliar token sequences scattered through the context and competes with
# the chat-template control tokens vLLM injects. The Gemma docs' own examples
# use plain-language sections with markdown headers, bold key terms, and
# numbered lists — so this port converts each block to that idiom while
# preserving every rule, tier, modifier, and example verbatim.
#
# Thinking mode is intentionally NOT enabled (no `<|think|>` token in the
# system prompt). Adding it would emit `<|channel>thought…<channel|>` ahead
# of the JSON body and break `parse_extraction_response` (which does
# `json.loads(text.strip())`) and conflict with vLLM's `guided_json` decoder.
# If a future run wants thinking, that should be a chat-template kwarg on the
# vLLM side, not a prompt-content change.

_SYSTEM_PROMPT_GEMMA_V4: Final[str] = _SYSTEM_PROMPT_V4

_EXTRACTION_INSTRUCTIONS_GEMMA_V4: Final[str] = """\
# Task

Extract every gene with putative causal links to cerebral small vessel disease (cSVD) from the research paper that follows. Return a single JSON object matching the schema enforced by the runtime — no prose, no markdown wrappers, no commentary outside of the JSON.
...
### Example — include_high_confidence
...
### Example — include_strong_functional
...
### Example — include_gwas_with_twas
...
### Example — include_twas_causal_gene

**Paper states:** "At the WM-PVS locus 6p25.2, the lead SNP is near LINC01600. TWAS analysis identified C6orf195 as transcriptome-wide significant (p = 2.8e-6) with colocalization PP4 > 0.75."

**Result:** `gene_symbol="C6orf195"`, `gwas_trait=["WM-PVS"]`, `omics_evidence=["TWAS", "colocalization"]`, `confidence=0.75`

**Reasoning:** Do NOT extract LINC01600 — it is the positional locus label, not the causal gene identified by TWAS/colocalization.

### Example — exclude_positional_candidate

**Paper states:** "At the WM-PVS locus on chromosome 1q25, we identified 8 positional candidate genes: LAMC1, EFEMP1, LPAR1, ITGB5, RGL1, CACNA1E, NOS1AP, and KCNT2. TWAS analysis identified ITGB5 as the only gene with significant brain expression association (p = 3.1e-7)."

**Reasoning:** Extract ONLY ITGB5 (with TWAS evidence). Do NOT extract LAMC1, EFEMP1, LPAR1, RGL1, CACNA1E, NOS1AP, or KCNT2 — these are positional-only candidates with no individual gene-level evidence. Being listed in a locus table is NOT sufficient.

### Example — exclude_prior_literature_gene

**Paper states:** "FOXF2 has been previously implicated in SVS through GWAS (Chauhan et al., 2019). SLC20A2 is a known cause of familial brain calcification. WNT7A has been linked to blood-brain barrier development."

**Reasoning:** Do NOT extract FOXF2, SLC20A2, or WNT7A. These are citations of prior literature findings. This paper does not present new statistical or functional data for these genes.

### Example — include_orf_gene
...
### Example — exclude_general_stroke

**Paper states:** "PITX2 and ZFHX3 reached genome-wide significance for cardioembolic stroke in MEGASTROKE. We included these in our comparison of stroke subtypes."

**Reasoning:** Do NOT extract. PITX2 and ZFHX3 are cardioembolic-specific. No evidence for SVS, WMH, or any cSVD phenotype.

### Example — exclude_pathway_only

**Paper states:** "Gene set enrichment analysis identified the extracellular matrix pathway (GO:0031012) as significantly enriched among our GWAS hits. This set includes 47 genes including FBN1, LAMA2, and COL6A3."

**Reasoning:** Do NOT extract FBN1, LAMA2, COL6A3. These genes are mentioned only as members of an enriched pathway. No individual-level statistical significance or causal evidence is presented for these specific genes.

### Example — include_low_confidence_pqtl_mr
...
### Example — exclude_background_monogenic

**Paper states:** "CADASIL, caused by NOTCH3 mutations, is the most common monogenic form of cSVD. In our GWAS of WMH volume, we identified 20 novel loci."

**Reasoning:** Do NOT extract NOTCH3. This is a background citation of a known monogenic gene. The paper does not present new NOTCH3 data. Only extract the novel GWAS loci with their specific evidence.

# Output format

Return a single JSON object shaped as `{"genes": [...]}`. Each entry uses the field names listed under **Field guidance**. Output JSON only — no code fences, no commentary before or after. The runtime enforces the schema, so any prose outside the JSON will be rejected."""

# ---------------------------------------------------------------------------
# Gemma 4 v5 prompts (multi-phenotype convergence + MTAG locus scanning)
# ---------------------------------------------------------------------------
# Same persona as v4 — only the instructions body diverges, mirroring how
# _SYSTEM_PROMPT_V5 reuses _SYSTEM_PROMPT_V4 above.

_SYSTEM_PROMPT_GEMMA_V5: Final[str] = _SYSTEM_PROMPT_V4

_EXTRACTION_INSTRUCTIONS_GEMMA_V5: Final[str] = """\
# Task
...
### Example — include_multi_phenotype
