...
for row in rows:
    csv_pop = row["SVD Population"]
    display_pop = pop_name_mapping.get(csv_pop, csv_pop)
    pop_to_rows[display_pop].append(row)

# ---------- ANGLES PROPORTIONAL TO UNIQUE DRUG COUNTS ----------
pop_counts = {}
for pop in populations:
    drugs = set()
    for row in pop_to_rows[pop]:
        name = row.get("Drug", "").strip()
        if name:
            drugs.add(name)
...
for ph in phase_label_render_data:
    svg_parts.append(
        shadow_rect(
            ph["box_x"],
            ph["box_y"],
            ph["est_width"] + ph["padX"] * 2,
            ph["est_height"] + ph["padY"] * 2,
        )
    )
...
# Collect unique mechanisms, preserving insertion order
mechanisms: list[str] = []
seen_mechs: set[str] = set()
for row in rows:
    mech = (row.get("Mechanism of Action") or "").strip()
    if mech and mech not in seen_mechs:
        seen_mechs.add(mech)
...
for pop in populations:
    ...
    for phase, rows_in_phase in phase_to_rows.items():
        ...
        for j, row in enumerate(rows_in_phase):
            ...
            x, y = pol2cart(r_mid, theta)
            anchor = "start" if math.cos(theta) >= 0 else "end"

            if math.cos(theta) >= 0:
                lx = x + MARKER_R + LABEL_MARKER_GAP
            else:
                lx = x - MARKER_R - LABEL_MARKER_GAP
            ly = y
...
for d in drug_render_data:
    ...
    svg_parts.append(
        shadow_rect(
            d["box_x"],
            d["box_y"],
            d["est_width"] + d["padX"] * 2,
            d["est_height"] + d["padY"] * 2,
        )
    )
...
legend_center_x = legend_x + legend_width / 2 - LEGEND_INSET_X
box_width = 40
legend_box_x = legend_center_x - box_width / 2
...
for mech in mechanisms:
    ...
    if paren_index != -1:
        lines = [mech[:paren_index].rstrip(), mech[paren_index:]]
...
legend_bg_x = legend_x - LEGEND_INSET_X
legend_bg_y = legend_y - LEGEND_INSET_Y
moa_bg_x = legend_moa_x - LEGEND_INSET_X
moa_bg_y = legend_moa_y - LEGEND_INSET_Y

# Layer 1: Legend background shadows
svg_parts.append(
    shadow_rect(legend_bg_x, legend_bg_y, legend_width, legend_height, rx=10)
)
svg_parts.append(
    shadow_rect(moa_bg_x, moa_bg_y, moa_legend_width, moa_legend_height, rx=10)
)
...
svg_parts.append(
    f'<g id="legend" font-size="14" fill="#000" font-family="Raleway">'
    f'<text x="{legend_center_x:.2f}" y="{legend_y - 12}" text-anchor="middle" font-size="15" font-weight="500">Genetic Evidence</text>'
    f'<g class="legend-item">'
    f'<rect x="{legend_box_x:.2f}" y="{legend_y + 2}" width="{box_width}" height="20" rx="6" '
    f'fill="#90ee90" fill-opacity="1"/>'
    f'<text x="{legend_center_x:.2f}" y="{legend_y + 12}" text-anchor="middle" dy="0.35em">Yes</text>'
    f"</g>"
    f'<g class="legend-item">'
    f'<rect x="{legend_box_x:.2f}" y="{legend_y + 30}" width="{box_width}" height="20" rx="6" '
...
# MOA legend content
svg_parts.append(
    f'<g id="legend-moa" font-size="14" fill="#000" font-family="Raleway">'
    f'<text x="{legend_moa_x + moa_legend_width / 2 - LEGEND_INSET_X:.2f}" y="{legend_moa_y - 12}" text-anchor="middle" font-size="15" font-weight="500">Mechanism of Action</text>'
)
...
""" + r"""  document.body.classList.add('svg-loading');
  ...
  <div style="padding:2px 4px;">
    ...
    nodes.forEach(node => {
      // Forward label-text clicks to the parent .drug node so the click
      // handler below fires from either the marker or the label.
